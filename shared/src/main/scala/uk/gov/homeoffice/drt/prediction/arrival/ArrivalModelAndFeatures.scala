...
trait ArrivalModelAndFeatures extends ModelAndFeatures {
  def dayOfWeek(ts: Long)(implicit sDateProvider: Long => SDateLike): String = s"dow_${sDateProvider(ts).getDayOfWeek()}"
  def amPm(ts: Long)(implicit sDateProvider: Long => SDateLike): String = s"dow_${sDateProvider(ts).getHours() / 12}"
