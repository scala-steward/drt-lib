...
object SDate {
  ...
  object implicits {
    ...
    implicit val sdateFromMillisLocal: MillisSinceEpoch => SDateLike = millis => SDate(millis, europeLondonTimeZone)

    implicit val sdateFromMilliDateLocal: MilliDate => SDateLike =
      milliDate => new DateTime(milliDate.millisSinceEpoch, europeLondonTimeZone)
  }
