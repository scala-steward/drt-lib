...
import uk.gov.homeoffice.drt.arrivals.{ApiFlightWithSplits, Arrival, ArrivalExportHeadings, Splits}
import uk.gov.homeoffice.drt.models.{AgeRange, FlightManifestSummary, UnknownAge}
import uk.gov.homeoffice.drt.ports.{FeedSource, SplitRatiosNs}
...
object FlightExports {
  ...
  def actualAPISplitsForFlightInHeadingOrder(flight: ApiFlightWithSplits, headings: Iterable[String]): Iterable[Double] =
    headings
      .map(h => actualAPISplitsAndHeadingsFromFlight(flight).toMap.getOrElse(h, 0.0))
